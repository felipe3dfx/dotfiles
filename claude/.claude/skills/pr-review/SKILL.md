...
|---------------|----------------|
| `apps/api/` (endpoints, schemas) | project API skill + `django-ninja` |
| `apps/*/tests/` (unit/integration tests) | project test skill + `pytest` |
| `e2e/`, `tests/e2e/` (end-to-end tests) | `playwright` |
| Templates with `hx-*` attributes | `htmx` |
| Templates with `x-data`, `x-bind`, Alpine.js | `alpinejs` |
| `components/` (django-components) | `django-components` |
| CSS, TailwindCSS classes, styling | `tailwind-4` |
| `tasks.py`, `@db_task`, `@db_periodic_task` | `huey` |
...
- Are naming conventions followed? (model names, manager methods, service functions, test file naming)
- Are the right patterns used? (service layer for business logic, managers for queries, CBVs for views)
- Do tests follow the project's test patterns? (Factory Boy, fixtures in conftest.py, `*_tests.py` naming)
...
- [ ] Test/debug files committed (`test.py`, `print()`, `breakpoint()`, etc.)
...
3. **Migration conflicts** - Multiple PRs creating migrations for the same app
4. **URL/view patterns** - New views need to be registered in `urls.py`

### Common Conflict Pattern: Migration Conflicts

When multiple PRs create migrations for the same Django app:
```python
# PR #1 creates: apps/insurance/migrations/0045_add_coverage_field.py
# PR #2 creates: apps/insurance/migrations/0045_add_status_field.py
# Both depend on 0044 → merge conflict!
```

**Flag this** - any PRs adding migrations to the same app will conflict.
