...
This skill is runtime-agnostic. Use your runtime's native delegation mechanism:

| Action | How |
|--------|-----|
| Launch parallel sub-agents | Two async delegations in a single step (e.g., `Agent` tool, `delegate`, `spawn`) |
| Wait for results | Read sub-agent output when both complete — never accept a partial verdict |
| Sequential delegation | Launch one sub-agent, wait for result, then proceed |
...
- The orchestrator NEVER reviews code itself — coordination only

### Pattern 2: Verdict Synthesis

The **orchestrator** (NOT a sub-agent) compares results after both agents complete:
...
1. If **confirmed issues** exist → launch a **Fix Agent** (separate sub-agent)
...
▼
Launch Judge A + Judge B in parallel — with Project Standards injected
▼
...
│   └── Launch Fix Agent with confirmed issues list
│       ▼
...
│       │
│       └── Still issues → Launch Fix Agent again (Round 3 / iteration 2)
│           ▼
...
│           │
│           ├── Clean → JUDGMENT: APPROVED
│           └── Still issues → JUDGMENT: ESCALATED (report to user)
```
...
| Race condition in worker.go:88 | found | — | WARNING | Suspect (A only) |
| Naming mismatch in handler.go:15 | — | found | SUGGESTION | Suspect (B only) |
| Error swallowed in db.go:201 | found | found | CRITICAL | Confirmed |
...
- Judge A: CLEAN — No issues found
- Judge B: CLEAN — No issues found
...
|---------|---------|---------|----------|
| {description} | found | found | CRITICAL |
...
- The **Fix Agent is a separate sub-agent** — never reuse a judge as the fixer
